...
onOK <- function(){
    ...
    command <- paste("lhs.design(","type=",dQuote(tclvalue(typerbVariable)),
        ...
        if (class(eval(parse(text=name)))[1]=="try-error") {
            errorCondition(window=topdes2,recall=Menu.lhs, message=gettextRcmdr(eval(parse(text=name))))
             return()
            }
...
onStore <- function(){
        ...
        storeRcmdr()
        assign(savename.RcmdrPlugin.DoE, getRcmdr(".stored.designlhs"), envir=.GlobalEnv)
...
dirVar <- tclVar(.stored.designlhs$dirVar)
